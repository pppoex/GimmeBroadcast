 ...
 *
 * Restores the UDP LAN broadcast that Forge disables by default.
 */
...
public class GimmeBroadcast {
    ...
    public GimmeBroadcast() { MinecraftForge.EVENT_BUS.register(this); LOG.info("{} registered", MOD_ID); }

    @SubscribeEvent
    public void onServerStarted(ServerStartedEvent event) {
        this.server = event.getServer();
        LOG.info("Server started – dedicated={}, published={}, port={}", server.isDedicatedServer(), server.isPublished(), server.getPort());
    ...
    @SubscribeEvent
    public void onServerStopping(ServerStoppingEvent event) { stopBroadcast(); this.server = null; }
    ...
    private void stopBroadcast() { if (broadcastThread != null) { broadcastThread.stopBroadcast(); broadcastThread = null; } }
}
